...
| 路线 | 何时用 | 说明 |
|---|---|---|
| **主路线：MinerU（本地）** | 默认，一切常规书籍 | `vlm-engine`＋MLX 加速（默认），Apple Silicon 本地推理，数据不出机——**隐私材料唯一合规路线**。环境在 `~/tools/pdf2md/.venv`，没有就先跑 `scripts/setup.sh` |
| **云端路由（双服务商）** | 公开出版物批量赶时间 | `cloud_digitize.py <PDF> <目录根> [--provider mineru\|baidu]`，产物统一落 `<书名>/cloud/`，修补/质检/索引链直接续接。**整本上传云端——隐私材料禁用** |
| ├ mineru（默认，免费） | 日常首选 | 2000 页/日免费高优额度，超 200 页自动切段合并（实测 266 页约 2 分钟出全书）。需 `MINERU_API_TOKEN`（mineru.net 免费注册） |
| └ baidu（付费兜底） | **mineru 当日额度用尽或不可用时推荐用户切换** | PaddleOCR-VL（OmniDocBench 榜首），约 9 元/千页、1000 页免费测试，≤500 页/段自动切分。需 `BAIDU_OCR_AK/SK`（百度智能云开通"文档解析"）。通道按官方文档实现，**首跑留意字段验证**。它同时也是 crosscheck.py 的第二意见引擎 |
